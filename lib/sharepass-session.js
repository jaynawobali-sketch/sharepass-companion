...
export const SESSION_KEYS = {
  ...
  voiceMemberId: "sharepass.voiceMemberId",
...
export function readStoredTheme(storage, fallback = "dark") {
  const storedTheme = storage?.getItem?.(SESSION_KEYS.theme);
...
export function ensureSuperAdminEmails(storage, sessionProfile) {
  return getConfiguredSuperAdminEmails();
}
...
export function clearSharePassSession(storage) {
  ...
  storage.removeItem(SESSION_KEYS.superAdmins);
}
