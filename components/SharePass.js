    ...
    try {
      ...
      setVoiceError(
        /permission|denied|blocked/i.test(message)
          ...
          : /NotFound|DeviceNotFound/i.test(error?.name || "")
            ? "No microphone found. Check that your microphone is connected."
            : /NotSupported|NotAllowed/i.test(error?.name || "")
              ? "This browser cannot access your microphone. Try Chrome or Safari."
    ...
    Object.entries(remoteAudioRefs.current).forEach(([memberId, node]) => {
      ...
      if (remoteStream) {
        ...
        if (maybePromise && typeof maybePromise.then === "function") {
          maybePromise
            .then(() => {
              console.info("[voice] remote audio playing successfully", memberId);
            ...
            .catch(error => {
              const errorInfo = {
                ...
                trackDetails: remoteStream.getAudioTracks().map(t => ({
                  id: t.id,
                  enabled: t.enabled,
                  muted: t.muted,
              ...
              if (/NotAllowed|NotSupported|Permission/i.test(error?.name || "")) {
                pendingPlaybackMembersRef.current.add(memberId);
                setAutoplayBlocked(true);
                console.warn("[voice] remote audio autoplay blocked (will retry on user interaction)", errorInfo);
              } else {
                console.error("[voice] remote audio play error", errorInfo);
              }
            });
        } else if (!maybePromise) {
          console.warn("[voice] play() returned no promise - old browser?", { memberId });
          setPipelineStage("playback");
        }
  ...
  useEffect(() => {
    ...
    const retryPlayback = () => {
      ...
      pendingPlaybackMembersRef.current.forEach(memberId => {
        ...
        if (!node?.srcObject) {
          console.warn("[voice] no srcObject for", memberId, "- skipping retry");
          pendingPlaybackMembersRef.current.delete(memberId);
          return;
        }

        console.info("[voice] attempting playback retry for", memberId);
        const maybePromise = node.play?.();
        if (maybePromise && typeof maybePromise.then === "function") {
          maybePromise
            .then(() => {
              console.info("[voice] remote audio resumed after user interaction", { memberId });
              pendingPlaybackMembersRef.current.delete(memberId);
              if (pendingPlaybackMembersRef.current.size === 0) {
                setAutoplayBlocked(false);
              }
            ...
            .catch((error) => {
              console.error("[voice] playback retry failed for", memberId, ":", error?.message);
            });
        } else {
          console.warn("[voice] play() returned no promise on retry for", memberId);
        }
      });
    };
